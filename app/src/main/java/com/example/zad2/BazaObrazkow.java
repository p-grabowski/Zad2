package com.example.zad2;

import android.net.Uri;

import java.util.ArrayList;
import java.util.List;

public class BazaObrazkow {

    public static List<String> tittle = new ArrayList<>();
    public static List<String> subtittle = new ArrayList<>();
    public static List<String> path = new ArrayList<>();
    public static List<String> path_photo = new ArrayList<>();
    public static List<String> audioSecond = new ArrayList<>();
    public static List<Uri> uri = new ArrayList<>();
    public static List<Uri> uri_photo = new ArrayList<>();

}
